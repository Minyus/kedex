...
class CatalogSyntacticSugarContext(KedroContext):
    ...
    def _format_kedro_catalog(self, conf_catalog):
        ...
        for ds_name, ds_dict_ in conf_catalog.items():
            ...
            ds_name, ds_dict = _format_kedro_dataset(ds_name, ds_dict)
            conf_catalog_processed[ds_name] = ds_dict
        return conf_catalog_processed


def _format_kedro_dataset(ds_name, ds_dict):
    _check_type(ds_dict)
    ds_name, ds_dict = _set_filepath(ds_name, ds_dict)
    ds_name, ds_dict = _enable_caching(ds_name, ds_dict)
    return ds_name, ds_dict


def _set_filepath(ds_name, ds_dict):
